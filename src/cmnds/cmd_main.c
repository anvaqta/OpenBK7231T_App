...
void LN882H_ApplyPowerSave(int bOn) {
  if (bOn) {
    ...
    } else { // to be able to switch from PowerSave from > 1 to 1 (without
             // sleep)
...
static commandResult_t CMD_PowerSave(const void *context, const char *cmd,
...
#if (PLATFORM_LN882H)
  ADDLOG_INFO(LOG_FEATURE_CMD, "CMD_PowerSave: will set to %i%s", bOn,
              Main_IsConnectedToWiFi() == 0 ? " after WiFi is connected" : "");
#else
  ...
  if (Tokenizer_GetArgsCount() > 1) {
    ...
    if (tx > maxtx / 4) {
      ADDLOG_ERROR(LOG_FEATURE_CMD,
                   "TX power maximum is: %ddBm, entered: %idBm", maxtx / 4, tx);
    } else {
  ...
  if (Tokenizer_GetArgsCount() > 3) {
    ...
    ADDLOG_INFO(LOG_FEATURE_CMD,
                "PowerSave freq scaling, min: %iMhz, max: %iMhz", minfreq,
                maxfreq);
  }
#endif
#elif PLATFORM_REALTEK
  if (!wifi_is_up(RTW_STA_INTERFACE)) {
    ADDLOG_ERROR(
        LOG_FEATURE_CMD,
        "Wifi is not on or in AP only mode, failed setting powersave!");
    g_powersave = (bOn);
  ...
  if (bOn) {
    ...
    if (bOn >= 2) {
      ...
      rtw_reduce_pa_gain =
          bOn == 2 ? 1 : 2; // 0:not reduce, 1:reduce 2, 2:reduce 3
...
static commandResult_t CMD_DeepSleep(const void *context, const char *cmd,
                                     const char *args, int cmdFlags) {
  int timeMS;
  ...
  //  this works fine, but wifi will not receive anything after waking up, and
  //  only full power cycle will fix it up.
...
static commandResult_t CMD_ScheduleHADiscovery(const void *context,
                                               const char *cmd,
                                               const char *args, int cmdFlags) {
  int delay;
...
#endif
static commandResult_t CMD_SetFlag(const void *context, const char *cmd,
                                   const char *args, int cmdFlags) {
  int flag, val;
...
static commandResult_t CMD_Flags(const void *context, const char *cmd,
                                 const char *args, int cmdFlags) {
  union {
    long long newValue;
    struct {
  ...
  // ADDLOG_INFO(LOG_FEATURE_CMD, "CMD_Flags: sizeof(newValue) = %i",
  // sizeof(u.newValue));
  if (args && *args) {
    ...
    // ADDLOG_INFO(LOG_FEATURE_CMD, "CMD_Flags: 0-31: %X 32-63: %2 = %X",
    // u.ints[0], u.ints[1]);
    u.newValue = (uint64_t)strtoull(args, NULL, 10);
    // ADDLOG_INFO(LOG_FEATURE_CMD, "CMD_Flags (new): 0-31: %X 32-63: %2 = %X",
    // u.ints[0], u.ints[1]);
  } else {
    return CMD_RES_NOT_ENOUGH_ARGUMENTS;
  }
...
static commandResult_t CMD_HTTPOTA(const void *context, const char *cmd,
                                   const char *args, int cmdFlags) {
...
static commandResult_t CMD_Restart(const void *context, const char *cmd,
                                   const char *args, int cmdFlags) {
  int delaySeconds;
...
static commandResult_t CMD_ClearAll(const void *context, const char *cmd,
                                    const char *args, int cmdFlags) {
  ...
  RepeatingEvents_Cmd_ClearRepeatingEvents(0, 0, 0, 0);
    ...
    defined(PLATFORM_LN882H) || defined(PLATFORM_ESPIDF) ||                    \
    defined(PLATFORM_TR6260) || defined(PLATFORM_REALTEK)
...
static commandResult_t CMD_ClearNoPingTime(const void *context, const char *cmd,
                                           const char *args, int cmdFlags) {
  g_timeSinceLastPingReply = 0;
  return CMD_RES_OK;
}
static commandResult_t CMD_ClearConfig(const void *context, const char *cmd,
                                       const char *args, int cmdFlags) {
  ...
  ADDLOG_INFO(LOG_FEATURE_CMD,
              "CMD_ClearConfig: whole device config has been cleared, restart "
              "device to connect to clear AP");

  return CMD_RES_OK;
}
static commandResult_t CMD_ClearIO(const void *context, const char *cmd,
                                   const char *args, int cmdFlags) {
...
static commandResult_t CMD_Echo(const void *context, const char *cmd,
                                const char *args, int cmdFlags) {
  ...
  Tokenizer_TokenizeString(args, TOKENIZER_ALTERNATE_EXPAND_AT_START |
                                     TOKENIZER_FORCE_SINGLE_ARGUMENT_MODE);
...
static commandResult_t CMD_StartupCommand(const void *context, const char *cmd,
                                          const char *args, int cmdFlags) {
  const char *cmdToSet;
...
static commandResult_t CMD_Choice(const void *context, const char *cmd,
                                  const char *args, int cmdFlags) {
  int indexToUse;
  const char *cmdToUse;
...
static commandResult_t CMD_PingHost(const void *context, const char *cmd,
                                    const char *args, int cmdFlags) {
  Tokenizer_TokenizeString(args, 0);
...
static commandResult_t CMD_PingInterval(const void *context, const char *cmd,
                                        const char *args, int cmdFlags) {
  Tokenizer_TokenizeString(args, 0);
...
static commandResult_t CMD_SetStartValue(const void *context, const char *cmd,
                                         const char *args, int cmdFlags) {
  int ch, val;
...
static commandResult_t CMD_OpenAP(const void *context, const char *cmd,
                                  const char *args, int cmdFlags) {
...
static commandResult_t CMD_SafeMode(const void *context, const char *cmd,
                                    const char *args, int cmdFlags) {
  int i;
  int startSaveModeIn;
...
static commandResult_t runcmd(const void *context, const char *cmd,
                              const char *args, int cmdFlags) {
  char *c = (char *)context;
  //   char *p = c;
...
commandResult_t CMD_CreateAliasHelper(const char *alias, const char *ocmd) {
  ...
  if (existing != 0) {
    ADDLOG_INFO(LOG_FEATURE_EVENT,
                "CMD_Alias: the alias you are trying to use is already in use "
                "(as an alias or as a command)");
    return CMD_RES_BAD_ARGUMENT;
  }
  ...
  ADDLOG_INFO(LOG_FEATURE_CMD, "New alias has been set: %s runs %s", alias,
              ocmd);
...
// run an aliased command
static commandResult_t CMD_CreateAliasForCommand(const void *context,
                                                 const char *cmd,
                                                 const char *args,
                                                 int cmdFlags) {
  const char *alias;
                ...
                for (i = 0; i < readlen; i++) {
                        if (buffer[i] == data[matching]) {
                                ...
                                if (matching == dataSize) {
                                        free(buffer);
                                        return at + i - dataSize + 1;  //
Pattern found, return the offset
                                }
                        }
                        else {
                                matching = 0;
...
/*
commandResult_t CMD_FindPattern(const void *context, const char *cmd, const char
*args, int cmdFlags) { int startOfs, endOfs; int maxData; int realDataSize;
        const char *hexStr;
        byte b;
        byte *data;
        int result;
...
commandResult_t CMD_PWMFrequency(const void *context, const char *cmd,
                                 const char *args, int cmdFlags) {
  Tokenizer_TokenizeString(args,
                           TOKENIZER_ALLOW_QUOTES | TOKENIZER_DONT_EXPAND);
  // following check must be done after 'Tokenizer_TokenizeString',
  // so we know arguments count in Tokenizer. 'cmd' argument is
  // only for warning display
  if (Tokenizer_CheckArgsCountAndPrintWarning(cmd, 1)) {
  ...
  esp_err_t err = ledc_set_freq(LEDC_LOW_SPEED_MODE, LEDC_TIMER_0,
                                (uint32_t)g_pwmFrequency);
  if (err == ESP_ERR_INVALID_ARG) {
    ADDLOG_ERROR(LOG_FEATURE_CMD, "ledc_set_freq: invalid arg");
    return CMD_RES_BAD_ARGUMENT;
  } else if (err == ESP_FAIL) {
    ADDLOG_ERROR(LOG_FEATURE_CMD,
                 "ledc_set_freq: Can not find a proper pre-divider number base "
                 "on the given frequency and the current duty_resolution");
    return CMD_RES_ERROR;
  }
#endif
...
commandResult_t CMD_IndexRefreshInterval(const void *context, const char *cmd,
                                         const char *args, int cmdFlags) {
  Tokenizer_TokenizeString(args, 0);
  // following check must be done after 'Tokenizer_TokenizeString',
  // so we know arguments count in Tokenizer. 'cmd' argument is
...
commandResult_t CMD_DeepSleep_SetEdge(const void *context, const char *cmd,
                                      const char *args, int cmdFlags) {
  ...
  if (Tokenizer_GetArgsCount() > 1) {
    // DSEdge [Edge] [Pin]
    PIN_DeepSleep_SetWakeUpEdge(Tokenizer_GetArgInteger(1),
...
#if MQTT_USE_TLS
static commandResult_t CMD_WebServer(const void *context, const char *cmd,
                                     const char *args, int cmdFlags) {
  int arg_count;
  Tokenizer_TokenizeString(args, 0);
...
void CMD_Init_Early() {
  ...
  // cmddetail:"descr":"add an aliased command, so a command with spaces can be
  // called with a short, nospaced alias",
  ...
  // cmddetail:"descr":"Sends given message back to console. This command
  // expands variables, so writing $CH12 will print value of channel 12, etc.
  // Remember that you can also use special channel indices to access persistant
  // flash variables and to access LED variables like dimmer, etc.",
  ...
  // cmddetail:"descr":"Same as restart. Needed for bkWriter 1.60 which sends
  // 'reboot' cmd before trying to get bus via UART. Thanks to this, if you
  // enable command line on UART1, you don't need to manually reboot while
  // flashing via UART.",
  // cmddetail:"fn":"CMD_Restart","file":"cmnds/cmd_main.c","requires":"",
  // cmddetail:"examples":""}
  ...
  // cmddetail:"descr":"Clears config and all remaining features, like runtime
  // scripts, events, etc",
  ...
  // cmddetail:"descr":"Starts deep sleep for given amount of seconds. Please
  // remember that there is also a separate command, called PinDeepSleep, which
  // is not using a timer, but a GPIO to wake up device.",
  // cmddetail:"fn":"CMD_DeepSleep","file":"cmnds/cmd_main.c","requires":"",
  // cmddetail:"examples":""}
  CMD_RegisterCommand("DeepSleep", CMD_DeepSleep, NULL);
  // cmddetail:{"name":"PowerSave","args":"[Optional 1 or 0, by default 1 is
  // assumed]", cmddetail:"descr":"Enables dynamic power saving mode on all
  // platforms with the exception of TR6260. On LN882H PowerSave 1 = light sleep
  // and Powersave >1 (eg PowerSave 2) = deeper sleep. On LN882H PowerSave 1
  // should be used if BL0937 metering is present. On all supported platforms
  // PowerSave 0 can be used to disable power saving.",
  ...
  // cmddetail:"descr":"Executed a conditional. Condition should be single line.
  // You must always use 'then' after condition. 'else' is optional. Use aliases
  // or quotes for commands with spaces",
  ...
  // cmddetail:"descr":"Starts the firmware update procedure, the argument
  // should be a reachable HTTP server file. You can easily setup HTTP server
  // with Xampp, or Visual Code, or Python, etc. Make sure you are using OTA
  // file for a correct platform (getting N platform RBL on T will brick device,
  // etc etc)",
  ...
  // cmddetail:"descr":"This will schedule HA discovery, the discovery will
  // happen with given number of seconds, but timer only counts when MQTT is
  // connected. It will not work without MQTT online, so you must set MQTT
  // credentials first.",
  ...
  // cmddetail:"descr":"Command for ping watchdog; it sets the 'time since last
  // ping reply' to 0 again",
  ...
  // cmddetail:"descr":"Sets the startup value for a channel. Used for start
  // values for relays. Use 1 for High, 0 for low and -1 for 'remember last
  // state'",
  ...
  // cmddetail:"descr":"Temporarily disconnects from programmed WiFi network and
  // opens Access Point",
  ...
  // cmddetail:"descr":"DeepSleep (PinDeepSleep) wake configuration command. 0
  // means always wake up on rising edge, 1 means on falling, 2 means if state
  // is high, use falling edge, if low, use rising. Default is 2. Second
  // argument is optional and allows to set per-pin DSEdge instead of setting it
  // for all pins.",
  ...
  // cmddetail:"descr":"Forces device reboot into safe mode (open ap with
  // disabled drivers). Argument is a delay to restart in seconds, optional,
  // minimal delay is 1",
  ...
  // cmddetail:"descr":"Sets or gets the safe mode timeout value. If no argument
  // is provided, returns the current timeout value. If an argument is provided,
  // sets the timeout to that value.",
  ...
  // cmddetail:"descr":"Sets the interval between ping attempts for ping
  // watchdog mechanism",
...
#endif
  // cmddetail:{"name":"StartupCommand","args":"[Command in quotation
  // marks][bRunAfter]", cmddetail:"descr":"Sets the new startup command (short
  // startup command, the one stored in config) to given string. Second argument
  // is optional, if set to 1, command will be also executed after setting",
  ...
  // cmddetail:"descr":"This will choose a given argument by index and execute
  // it as a command. Index to execute can be a variable like $CH1.",
...
#endif
#if PLATFORM_BEKEN || WINDOWS || PLATFORM_BL602 || PLATFORM_ESPIDF ||          \
    PLATFORM_ESP8266 || PLATFORM_REALTEK || PLATFORM_ECR6600 ||                \
    PLATFORM_XRADIO
  UART_AddCommands();
#endif
#if ENABLE_BL_TWIN
...
void CMD_ListAllCommands(void *userData,
                         void (*callback)(command_t *cmd, void *userData)) {
  int i;
  command_t *newCmd;
...
command_t *CMD_RegisterCommand(const char *name, commandHandler_t handler,
                               void *context) {
  int hash;
  command_t *newCmd;
  ...
  if (newCmd != 0) {
    // it happens very often in Simulator due to the lack of the ability to
    // remove commands
    if (newCmd->handler != handler) {
      ADDLOG_ERROR(LOG_FEATURE_CMD, "command with name %s already exists!",
                   name);
    }
    return 0;
  }
...
commandResult_t CMD_ExecuteCommandArgs(const char *cmd, const char *args,
                                       int cmdFlags) {
  command_t *newCmd;
  // int len;
